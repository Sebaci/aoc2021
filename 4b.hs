...
solve boards numbers =
  lastCalled * score (solvedBingos !! lastBoardIndex)
  where
    Just lastBoardIndex = findIndex (not . bingo) $ (snd . last) unsolvedStates
    (unsolvedStates, (Just lastCalled, solvedBingos) : _) = break (all bingo . snd) bingoHistory
